...
def CookieConsentBanner(base_path: str = "", reset_token: str = ""):
    """
    Cookie consent banner component.
    ...
    """
    reset_token_js = repr(reset_token or "")
        ...
        Div(
            ...
            Script("""
                function acceptCookies() {
                    ...
                    if (%(reset_token)s) localStorage.setItem('cookie_consent_reset_token', %(reset_token)s);
                    ...
                    var tab = document.getElementById('cookie-consent-tab');
                ...
                function openCookieConsent() {
                    ...
                    if (banner) {
                        ...
                        if (footer) footer.style.bottom = banner.offsetHeight + 'px';
                    }
                ...
                setTimeout(function() {
                    ...
                    var footer = document.getElementById('main-footer');
                    var tab = document.getElementById('cookie-consent-tab');
                    var resetToken = %(reset_token)s;

                    if (resetToken) {
                        var lastToken = localStorage.getItem('cookie_consent_reset_token') || '';
                        if (lastToken !== resetToken) {
                            localStorage.removeItem('cookie_consent');
                            localStorage.removeItem('cookie_consent_date');
                            document.cookie = 'cookie_consent=; path=/; max-age=0; SameSite=Lax';
                            localStorage.setItem('cookie_consent_reset_token', resetToken);
                        }
                    }
            ...
            """ % {"reset_token": reset_token_js}),
            
            id="cookie-consent-banner",
            cls="fixed bottom-0 left-0 right-0 bg-base-100 border-t border-base-300 p-4 shadow-lg z-50"
        ),
        Button(
            UkIcon("cookie", width="18", height="18"),
            onclick="openCookieConsent()",
            id="cookie-consent-tab",
            style="display:none",
            cls="btn btn-circle btn-ghost fixed left-4 bottom-24 z-50",
        ),
    )
